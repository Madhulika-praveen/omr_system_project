...
def clean_bubbles_with_ml(bubbles, sheet_img, model_path="E:\omr_system_project\models/bubble_classifier.pkl"):
    """
...
def extract_bubbles(sheet_image, grid_coordinates):
    detected = {}
    options = ['a', 'b', 'c', 'd']
    # absolute question numbers start for each subject
    ...
    for subject, bubbles in grid_coordinates.items():
        ...
        for q_index, start_idx in enumerate(range(0, len(bubbles), 4)):
            q_no = q_start + q_index  # absolute question number
            answer = ' '
...
def generate_grid_coordinates(bubbles, questions_per_subject=20):
    ...
    start_q = [1, 21, 41, 61, 81]  # absolute numbering per your CSV
    ...
    # assign bubbles to subjects
    grid_coordinates = {}
    idx = 0
    for subj_idx, subject in enumerate(subjects):
        subject_bubbles = []
        q_num_offset = start_q[subj_idx]
        for q_index in range(questions_per_subject):
            if idx < len(row_groups):
                row = row_groups[idx]
                # attach absolute question numbers if needed (optional)
